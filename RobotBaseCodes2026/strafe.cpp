...
void Strafe::poll() {
  ...
  float dist_avg = 15.0;
  float angle_err = 0.0;
  float wall_dist = 15.0;

  if (tiller_->getTurnCount() % 2 == 0) {
    float long_ir_left = tiller_->_front_left_ir->readSensor();
    float long_ir_right = tiller_->_front_right_ir->readSensor();
    ...
    if (long_ir_left > 0.0 and long_ir_right > 0.0) {
      ...
      angle_err = -1*(long_ir_left - long_ir_right);  // simple angle error estimate from front/side IR difference
    }
  } else {
    float short_ir_left = tiller_->_rear_left_ir->readSensor();
    float short_ir_right = tiller_->_rear_right_ir->readSensor();

    float wall_dist = 8; // cm from wall
